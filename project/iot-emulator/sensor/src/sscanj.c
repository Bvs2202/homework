...
int sscanj(const char *str, const char *fmt, ...) {
  va_list args;
  va_start(args, fmt);
  ...
  while (fmt[fmt_idx] && str[str_idx]) {
    ...
    if (fmt[fmt_idx] == 'h' && fmt[fmt_idx + 1] == 'h' &&
      ...
      *x_ptr = (unsigned char)_atoui(str + str_idx, 16);
      count++;
    ...
    if (fmt[fmt_idx] == '%') {
      ...
      switch (fmt[fmt_idx]) {
        case 'd': {
          ...
          *d_ptr = (int)_atoui(str + str_idx, 10);
          count++;
